...
namespace Genji.Models.Workers
{
    public class HttpRecorder : IHttpRecorder
    {
        ...
        public HttpRecorder(string logpath = null, bool recordIncoming = false)
        {
            ...
            log = $"{logpath}Genji-Server-Log-{now.ToString("MM-dd-yy")}.txt";
        }
        ...
        {
            Console.ForegroundColor = ConsoleColor.Red;
            PrintAndWrite($"{ToTimeString(DateTime.Now)} [???] HTTP ???: Genji server crashed because of {e.ToString()}: " + e.Message);
